...
func UpdateEquipmentJctVersion(hostname string, mid string, newJct string, version string) error {
	return db.Model(&Equipment{}).Where("hostname = ? AND mid = ?", hostname, mid).Updates(&Equipment{
		Version: version,
		Jct:     newJct,
	}).Error
}
